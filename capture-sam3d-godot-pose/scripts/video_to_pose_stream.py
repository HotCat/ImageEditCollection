#!/usr/bin/env python3
"""Fit a human-motion video to a humanoid GLB and stream ``pose.frame`` FK.
...
and a small temporal solver retargets the fused result to the target GLB's
actual rest rig.  The finished frames are stored as JSON and can be replayed
...
def validate_motion_cache(motion: Mapping[str, Any]) -> None:
    """Reject stale delta-space caches before they can corrupt a Godot rig."""
    if motion.get("schema") != "godot-pose-motion-cache" or motion.get("version") != 1:
        raise ValueError("unsupported motion cache schema or version")
    if motion.get("rotation_space") != ROTATION_SPACE:
        raise ValueError(
            "motion cache is not marked as Godot 4 absolute-local bone rotations; "
            "do not replay legacy rest-relative delta caches"
        )
    frames = motion.get("frames")
    if not isinstance(frames, list) or not frames:
        raise ValueError("motion cache contains no frames")
    bone_count = int(motion.get("bone_count", 0))
    if bone_count <= 0:
        raise ValueError("motion cache has an invalid bone_count")
    for frame_index, frame in enumerate(frames):
        if not isinstance(frame, Mapping) or len(frame) != bone_count:
            raise ValueError(f"frame {frame_index} does not contain {bone_count} bones")
        for bone_name, value in frame.items():
            if not isinstance(value, Sequence) or isinstance(value, (str, bytes)) or len(value) != 4:
                raise ValueError(f"frame {frame_index} bone {bone_name!r} is not an xyzw quaternion")
            quaternion = np.asarray(value, dtype=np.float64)
            if not np.isfinite(quaternion).all() or abs(float(np.linalg.norm(quaternion)) - 1.0) > 1e-4:
                raise ValueError(f"frame {frame_index} bone {bone_name!r} is not normalized")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--nlf-model", type=Path,
                        help="NLF TorchScript model (required unless --stream-cache is used)")
    ...
    parser.add_argument("--sam3d-repo", type=Path,
                        help="official sam-3d-body checkout")
    parser.add_argument("--sam3d-checkpoint", type=Path,
                        help="SAM 3D Body checkpoint")
    parser.add_argument("--mhr-model", type=Path,
                        help="MHR TorchScript model used by SAM 3D Body")
    parser.add_argument("--sam3d-device", default="cpu")
    parser.add_argument("--target-glb", type=Path,
                        help="target skinned GLB whose imported rest rotations match Godot")
    parser.add_argument("--output-dir", type=Path,
                        help="cache directory; defaults beside the video under <stem>-pose-stream")
    parser.add_argument("--expected-bones", type=int, default=56,
                        help="fail unless the target skin has this many bones; use 0 to disable")
...
def validate_paths(args: argparse.Namespace) -> None:
    ...
    for label, path in (
        ...
        if path is None:
            raise RuntimeError(f"{label} is required unless --stream-cache is used")
        if not path.exists():
    ...
    if args.expected_bones < 0:
        raise RuntimeError("--expected-bones must be zero or a positive integer")
    if args.output_dir is None:
        args.output_dir = args.video.parent / f"{args.video.stem}-pose-stream"
...
def fit_motion(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.expected_bones and len(target.names) != args.expected_bones:
        raise RuntimeError(
            f"expected target skin to contain {args.expected_bones} bones, found {len(target.names)}"
        )
