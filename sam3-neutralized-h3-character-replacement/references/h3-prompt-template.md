...
```text
subject_definitions:
<Subject 1> is the replacement character shown in <Picture 1>. For a head-only edit, only the face, visible hair, hairline, and ears come from <Picture 1>; the source body, neck, clothing, and accessories remain protected.
...
<Subject 3> is the protected body/neck (for a head edit), girl, props, environment, captions, stickers, camera, lighting, and every non-matte pixel in <Video 1>.
...
summary:
[video editing + reference generation + audio reuse] Edit <Video 1> by replacing every pixel inside the neutral-gray silhouette with <Subject 1> from <Picture 1>. For a head-only edit, replace only the visible face and hair matte and keep the original neck-to-body transition. Transfer only <Subject 2>'s motion, expressions, gaze, lip timing, and prop interaction. Preserve <Subject 3>, every non-matte pixel, camera, framing, captions, stickers, duration, FPS, and <Audio 1>. The gray silhouette must disappear completely.

retention_analysis:
<Subject 1> (inside the matte throughout [Shot 1]): attribute_transfer - preserve the replacement identity and, for a head-only edit, face/hair appearance from <Picture 1> consistently through all views and occlusions; do not redraw the protected source neck or body.
...
The target is a realistic full-frame video at exactly [WIDTH]x[HEIGHT], [FPS] fps, and [DURATION] seconds. Treat the neutral-gray silhouette in <Video 1> as a precise animated hard edit matte. Every gray pixel and only the gray silhouette must become the new character; for a head-only edit this means face and visible hair only, with the original neck boundary protected. The gray color must never remain visible. Do not use the silhouette as an appearance reference.
...
Keep <Subject 3> completely unchanged. For a head-only edit, preserve the original neck, collar, body, and clothing at the matte's lower edge. Protected props and the girl's hands have priority if the matte touches them: preserve their original shape, texture, color, position, and contact shadows, and place the replacement naturally behind or beside them. Preserve captions, stickers, background, camera path, and framing through the final frame.
